#!/bin/bash
# 100-Round FL Pipeline Logit Poisoning PINN Guard Defense Runner
...
start_blockchain() {
    ...
    if lsof -Pi :$BLOCKCHAIN_PORT -sTCP:LISTEN -t >/dev/null 2>&1; then
        print_warning "Port $BLOCKCHAIN_PORT already in use"
    else
...
print_info "PINN Guard Defense Experiment Run Completed successfully!"
