...
async def score_model(trainer: str, cid: str):
    ...
    try:
        ...
        logger.error(f"Error loading or scoring model {cid}: {e}. Submitting score 0.0.")
        score = 0.0
...
def main():
    ...
    events = set()
    try:
        latest_events = async_contract.events.StartScoring().get_logs(
            fromBlock=max(0, w3.eth.block_number - 2000)
        )
        if latest_events:
            earliest_recent_block = min(e["blockNumber"] for e in latest_events[-10:])
            last_seen_block = max(0, earliest_recent_block - 5)
            logger.info(f"Starting scorer scan from block {last_seen_block} based on recent events (current block {w3.eth.block_number})")
        else:
            last_seen_block = max(0, w3.eth.block_number - 200)
            logger.info(f"No recent events found. Starting scorer scan from block {last_seen_block} (current block {w3.eth.block_number})")
    except Exception as e:
        logger.error(f"Error querying recent events: {e}. Defaulting to block 0.")
        last_seen_block = 0
    # wandb.init(
    #     project="unifyfl",
    #     config={
